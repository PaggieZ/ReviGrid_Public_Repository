classdef gridObj < handle
    ...
    methods
        ...
        function IDs = getUnitIDs(obj)
            for i = 1:obj.unitCount
                IDs(i) = string(obj.unitList{i}.ID);
            end
        end
        ...
        function reportGridStatus(obj)
            ...
            for i = 1:obj.unitCount
                ...
                str = str + "   -" + currUnit.SP.Port + ": " + currUnit.ID + newline;
            end
            fprintf(obj.FID, str);
        ...
        function updateStatusAll(obj)
            currUnit = obj.unitList{1};
            numOfVal = size(currUnit.status50, 2);
            for i = 1:obj.unitCount
                currUnit = obj.unitList{i};
                % disp(currUnit.ID);
                % disp(currUnit.SP.BytesAvailable);
                currUnit.updateStatus();
                obj.gridStatus(i,:) = currUnit.status;
                writematrix(currUnit.status, "statusData.xlsx", "Sheet", "Grid", "writeMode", "append");
